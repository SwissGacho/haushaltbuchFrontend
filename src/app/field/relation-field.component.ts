...
export class RelationFieldComponent extends ConnectedComponent implements OnInit, OnDestroy {
    ...
    filteredOptions$ = combineLatest([this.options$, this.filterText$]).pipe(
        map(([options, filter]) =>
            options.filter((opt) => opt.display_name.toLowerCase().includes(filter.toLowerCase()))
        )
    );
    isOpen = false;
    isLoading = false;
    ...
    fetchPossibleValues() {
        ...
        console.log(
            `${this.componentID} checking if should open on load:`,
            this.shouldOpenOnLoad,
            'or is loading:',
            this.isLoading
        );
    ...
    override handleMessages(message: IncomingMessage): void {
        ...
        if (message.type === MessageType.Object && this.isLoading) {
            ...
            if (this.shouldOpenOnLoad) {
                ...
                this.open();
                this.selectedIndex = this.currentFilteredOptions.findIndex(
                    (opt) => (opt.id === null && !this.value) || opt.id === this.value?.id
                );
    ...
    onKeyDown(event: KeyboardEvent) {
        ...
        if (event.key === 'ArrowDown') {
            ...
            if (
                this.selectedIndex >= 0 &&
                this.selectedIndex < this.currentFilteredOptions.length
            ) {
