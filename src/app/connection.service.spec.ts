...
class MockWebSocketSubject {
    pipe(op1: any, ...ops: any[]): MockWebSocketSubject {
        return this;
    }
...
class MockSubject {
    pipe(op1: any, ...ops: any[]): MockSubject {
        return this;
    }
    subscribe(observer?: Partial<rxjs.Observer<any>>): MockSubscription | null {
        return null;
    }
}
...
describe('ConnectionService', () => {
    ...
    function testGetNewConnection(
        ...
        const spyOnWebSocket = jest
            .spyOn(connectionService, 'webSocket')
            .mockReturnValue(mockWebSocketSubject as unknown as rxws.WebSocketSubject<Message>);
        ...
        expect(spyOnAddConnection.mock.calls).toEqual([
            [mockWebSocketSubject as unknown as rxws.WebSocketSubject<Message>, mockSubscriber],
        ]);
        ...
        if (arg1Next) {
            ...
            arg1Next(mockInMsg);
            expect(spyOnHandleHandshake.mock.calls).toEqual([
                [
                    mockInMsg,
                    {
    ...
    function testHandleHandshakeMessage(msg: Message, primary: boolean, error?: RegExp | string) {
        ...
        const spyOnNext = jest
            .spyOn(ConnectionService.loginBySessionTokenSubject, 'next')
            .mockReturnValue();
        ...
        if (msg.type == MessageType.Hello && msg.token) {
            ...
            if (arg0Next) {
                ...
                expect(spyOnSendMessage.mock.calls).toEqual([
                    [mockLoginMsg, mockContext.subscriber.componentID],
                ]);
            }
    ...
    it("should throw when session token doesn't match", () => {
        const mockWelcomeMessage = new WelcomeMessage({
            type: MessageType.Welcome,
            token: 'mockToken',
            ses_token: 'mockOtherSes',
        });
        testHandleHandshakeMessage(mockWelcomeMessage, false, /alien session/);
    });

    it('should handle ByeMessages', () => {
        const mockByeMessage = new ByeMessage({ type: MessageType.Bye, token: 'mockToken' });
