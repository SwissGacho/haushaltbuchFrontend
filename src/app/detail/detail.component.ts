...
export class DetailComponent extends ConnectedComponent implements OnInit {
    ...
    private onSelectedObjectChange(object: BoIdentifier | null) {
        ...
        // Keep field list when schema is unchanged so re-selecting objects of the same
        // type does not temporarily remove all dynamic fields.
        if (sameTypeSelection && this.objectSchema) {
            this.objectFields = Object.keys(this.objectSchema);
        } else {
            this.objectFields = [];
        }
